package jp.cyberjapan.webtis.handler.feature 
{
	/**
	 * ...
	 * @author k-hatakeyama
	 ...
	 */
	public class ExSelectFeaturesHandler extends ClickHandler
	{
		/**
		 ...
		 */
		private var _selectedStyle:Function = ExSelectFeaturesHandler.defaultSelectedStyle;
		 ...
		 */
		public function ExSelectFeaturesHandler(map:Map=null, active:Boolean=false, enableClickSelection:Boolean=true, enableBoxSelection:Boolean=true, enableOverSelection:Boolean=false) {
		...
		private function setSelectedStyle(feature:Feature):void {
			...
			// EX改修箇所
			// ExEditFeaturesのマウスオーバー時にisSelectedStyleをtrueに設定しているが、その後でフィーチャを選択した際に到達するこの関数内では
			// isSelectedStyleがfalseになっており、ハイライト色を記憶してしまうため選択解除時に色が戻らない
			// よって、style情報を保持する以下の処理をコメントアウトする
			if(feature.style && feature.style.isSelectedStyle == false) {
				//feature.originalStyle = feature.style;
				//trace("ExSelectFeaturesHandler setSelectedStyle " + feature.name + " featureのoriginalStyle変更");
			}
			...
			if (feature is PointFeature && !(feature is LabelFeature)) {
				...
				feature.style = (this.selectedStyle != null) ? this.selectedStyle(feature) : ExSelectFeaturesHandler.defaultSelectedStyle(feature);
			}
		}
