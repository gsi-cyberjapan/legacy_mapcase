package  jp.cyberjapan.webtis.handler.feature.draw
{
	...
	// hamas : 13.08.08 右クリック（ポリゴン）直前の描画点を削除
		protected function mouseRightClick(event:Event=null):void {
			...
			// ポリゴンの構成点が２点以上になったら
				var lring:LinearRing
					= (this._polygonFeature.geometry as Polygon).componentByIndex(0) as LinearRing;
				lring.removePoint(lastPoint);

				lastPoint = lring.getLastPoint();
				_lastPointLocation = new Location(lastPoint.x, lastPoint.y);
