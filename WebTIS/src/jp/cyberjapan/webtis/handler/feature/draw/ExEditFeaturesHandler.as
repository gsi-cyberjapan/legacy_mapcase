package jp.cyberjapan.webtis.handler.feature.draw 
{
	 ...
	 */
	public class ExEditFeaturesHandler extends ClickHandler
	{
		/**
		 ...
		 */
		private var _selectedStyle:Function = ExEditFeaturesHandler.defaultSelectedStyle;
		 ...
		 */
		public function ExEditFeaturesHandler(map:Map=null, active:Boolean=false, enableClickSelection:Boolean=true, enableBoxSelection:Boolean=true, enableOverSelection:Boolean=false) {
		...
		override protected function registerListeners():void {
			...
			if (this.map) {
				...
				this.map.addEventListener(FeatureEvent.FEATURE_CLICK, this.onClickFeature);	// その１
				this.map.addEventListener(FeatureEvent.FEATURE_SELECTED, this.onSelected);	// その２
		...
		override protected function unregisterListeners():void {
			...
			if (this.map) {
				...
				this.map.removeEventListener(FeatureEvent.FEATURE_OVER, this.onOver);
				this.map.removeEventListener(FeatureEvent.FEATURE_OUT, this.onOut);
				this.map.removeEventListener(FeatureEvent.FEATURE_CLICK, this.onClickFeature);	// その１
				this.map.removeEventListener(FeatureEvent.FEATURE_SELECTED, this.onSelected);	// その２
		...
		private function select(featuresToSelect:Vector.<Feature>, additiveMode:Boolean=false):void {
			...
			if (this.map && (featuresToSelect.length > 0)) {
				// EX改修箇所 ここでFEATURE_SELECTEDを呼ぶとウィンドウ表示が２回実行されることになるためコメントアウト
				/*　　
		...
		private function setSelectedStyle(feature:Feature):void {
			...
			feature.style = (this.selectedStyle != null) ? this.selectedStyle(feature) : ExEditFeaturesHandler.defaultSelectedStyle(feature);
		}
